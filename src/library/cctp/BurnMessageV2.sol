...
import { BurnMessage } from "src/library/cctp/BurnMessage.sol";
...
library BurnMessageV2 {
     ...
     * @param _hookData Optional hook data for processing on the destination
     * domain
    ...
    function _getFeeExecuted(bytes29 _message)
        internal
        pure
        returns (uint256)
    {
    ...
    function _getExpirationBlock(bytes29 _message)
        internal
        pure
        returns (uint256)
    {
    ...
    function _getHookData(bytes29 _message) internal pure returns (bytes29) {
        return
            _message.slice(HOOK_DATA_INDEX, _message.len() - HOOK_DATA_INDEX, 0);
    }
    ...
    function _validateBurnMessageFormat(bytes29 _message) internal pure {
        ...
        require(
            _message.len() >= HOOK_DATA_INDEX, "Invalid burn message: too short"
        );
    }
}
